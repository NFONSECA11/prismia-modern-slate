...
const AI_TAG_CONFIG: Record<AiTag, AiTagConfig> = {
  cancel: {
    ...
    textClass: "text-cyan-300 drop-shadow-[0_0_4px_rgba(34,211,238,0.6)]",
  },
  reschedule: {
      ...
      {tagConfig && (
        <Tooltip>
          <TooltipTrigger asChild>
            <span className={`inline-flex items-center justify-center h-4 min-w-[1.25rem] px-1 rounded border ${tagConfig.bgClass} ${tagConfig.textClass} text-[9px] font-extrabold leading-none cursor-default`}>
              {tagConfig.label}
            </span>
          </TooltipTrigger>
