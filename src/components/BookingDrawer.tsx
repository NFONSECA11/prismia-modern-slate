...
export function BookingDrawer({ booking, onClose, onConfirmed, logoUrl, logoAlt, mode: drawerMode = "details" }: BookingDrawerProps) {
  ...
  const effectiveConversationCollapsed = drawerMode === "conversation" ? false : conversationCollapsed;
              ...
              {iaOpType === "schedule" && (
                <>
                    <button
                      ...
                      disabled={scheduleSuggestMut.isPending || !effectiveProcedureId}
              ...
              {iaOpType === "reschedule" && (
                <>
                  ...
                  <button
                    ...
                    disabled={rescheduleSuggestMut.isPending}
                    className="text-xs font-medium px-3 py-2 rounded-lg gradient-primary text-primary-foreground disabled:opacity-40 disabled:cursor-not-allowed transition-all inline-flex items-center gap-1.5"
                  >
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 flex flex-col gap-5">
          ...
          >
            <button
              ...
              className="px-4 py-2.5 text-[11px] font-medium text-muted-foreground uppercase tracking-[0.12em] border-b border-border/60 flex items-center gap-2 hover:text-foreground transition-colors w-full text-left"
              style={{ background: "hsl(var(--surface-raised) / 0.6)" }}
              aria-expanded={!effectiveConversationCollapsed}
              aria-label={effectiveConversationCollapsed ? "Expandir conversa" : "Recolher conversa"}
            >
              <MessageSquare className="h-3.5 w-3.5 text-primary/70" />
              <span>Conversa</span>
              <span className="ml-auto inline-flex items-center gap-1.5 text-[10px] font-mono opacity-70">
                <span className="inline-flex items-center justify-center min-w-[20px] h-[18px] px-1.5 rounded-full bg-primary/15 text-primary border border-primary/20 normal-case tracking-normal">
                  {messages.length}
                </span>
                msgs
              </span>
            ...
            <div
              ...
              style={{
                minHeight: "150px",
                background:
                  "linear-gradient(180deg, hsl(var(--surface-elevated)) 0%, hsl(var(--surface-raised)) 100%)",
              }}
            >
              {messagesLoading ? (
                ...
                messages.map((msg, idx) => {
                  ...
                  return (
                    <div key={msg.id} className={`flex ${isBotFinal ? "justify-end" : "justify-start"}`}>
                      <div
                        className={`relative max-w-[82%] px-3.5 py-2 text-[13px] leading-relaxed whitespace-pre-wrap break-words shadow-sm ${
                          isBotFinal
                            ? "rounded-2xl rounded-br-md text-primary-foreground"
                            : "rounded-2xl rounded-bl-md text-foreground border border-border/60"
                        }`}
                        style={
                          isBotFinal
                            ? {
                                background:
                                  "linear-gradient(135deg, hsl(186 72% 42%) 0%, hsl(186 72% 32%) 100%)",
                              }
                            : { background: "hsl(var(--surface-elevated))" }
                        }
                      >
                        <div className="pr-10">
                          {content ? content : <span className="italic opacity-60">[sem conteúdo]</span>}
                        </div>
                        <span
                          className={`absolute bottom-1 right-2 text-[9px] font-mono leading-none ${
                            isBotFinal ? "text-white/70" : "text-muted-foreground/80"
                          }`}
                        >
                          {timeLabel}
                        </span>
