  ...
  return (
    <div className="min-h-screen relative" style={{ background: mainBg }}>
      ...
      >
        ...
        {mobileMenuOpen && (
          <div className="sm:hidden border-t border-border/60 px-1 py-3 space-y-2 animate-fade-in">
            {/* Company & Unit */}
            {company && (
      ...
      <main className={`relative z-10 ${zenMode ? "hidden" : ""} ${view === "agenda" ? "px-2 sm:px-6 pt-2 sm:pt-5 pb-2 space-y-2 sm:space-y-2" : "px-2 sm:px-6 py-2 sm:py-5 space-y-2 sm:space-y-5"} max-w-[1440px] mx-auto`}>
        ...
        {view === "table" && (
          <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2 rounded-xl border-2 px-2 sm:px-3 py-2 sm:py-2" style={{ background: isLandscape || isGradient ? "hsl(var(--filter-bar-bg) / 0.80)" : "hsl(var(--filter-bar-bg))", borderColor: "hsl(var(--filter-bar-bg))", backdropFilter: isLandscape || isGradient ? "blur(12px)" : undefined }}>
            {/* Search + result count */}
            <div className="flex items-center gap-2">
